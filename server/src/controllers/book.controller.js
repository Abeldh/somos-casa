...
export const bookController = {
  async getAll(req, res, next) {
    try { const { category, search, featured, page, limit } = req.query; return successResponse(res, await bookService.getAll({ category, search, featured: featured === 'true', page: parseInt(page) || 1, limit: parseInt(limit) || 12 })); } catch (e) { next(e); }
  },
  ...
  async update(req, res, next) { try { return successResponse(res, await bookService.update(req.params.id, req.body), 'Libro actualizado'); } catch (e) { next(e); } },
