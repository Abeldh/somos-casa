    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <div className="mt-8">{isAuthenticated?<div className="flex items-center gap-3"><div className="flex items-center border border-gray-200 rounded-lg"><button onClick={()=>setQty(q=>Math.max(1,q-1))} className="w-10 h-10 flex items-center justify-center hover:bg-gray-50 rounded-l-lg"><Minus className="w-4 h-4"/></button><span className="w-12 text-center font-semibold text-sm">{qty}</span><button onClick={()=>setQty(q=>Math.min(book.stock,q+1))} className="w-10 h-10 flex items-center justify-center hover:bg-gray-50 rounded-r-lg"><Plus className="w-4 h-4"/></button></div><Button onClick={()=>{addItem(book.id,qty);setQty(1);}} disabled={book.stock===0} className="flex items-center gap-2" size="lg"><ShoppingCart className="w-5 h-5"/>{book.stock===0?'Agotado':'Agregar al Carrito'}</Button></div>:<Link to="/login"><Button variant="outline" size="lg">Inicia sesión para comprar</Button></Link>}</div>
