...
export const bookService = {
  async getAll({ category, search, featured, page = 1, limit = 12 }) {
    ...
    return { books, total, pages: Math.ceil(total / limit), page };
  },
  ...
  async getCategories() {
    const results = await prisma.book.findMany({ where: { isActive: true, category: { not: null } }, select: { category: true }, distinct: ['category'] });
    return { categories: results.map((b) => b.category).filter(Boolean) };
  },
  ...
  async update(id, data) {
    if (data.title) {
      let slug = generateSlug(data.title);
      const exists = await prisma.book.findFirst({ where: { slug, id: { not: id } } });
      if (exists) slug = `${slug}-${Date.now().toString(36)}`;
      data.slug = slug;
    }
  ...
  async toggleFeatured(id) {
    ...
    if (!book) { const e = new Error('Libro no encontrado'); e.statusCode = 404; throw e; }
    const updated = await prisma.book.update({ where: { id }, data: { isFeatured: !book.isFeatured } });
    return { book: updated };
  },
};
