...
export const cartController = {
  ...
  async addItem(req, res, next) { try { const { bookId, quantity } = req.body; return successResponse(res, await cartService.addItem(req.user.id, bookId, quantity || 1), 'Agregado al carrito'); } catch (e) { next(e); } },
  ...
  async removeItem(req, res, next) { try { return successResponse(res, await cartService.removeItem(req.user.id, req.params.id), 'Item eliminado'); } catch (e) { next(e); } },
